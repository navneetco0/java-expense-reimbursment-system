...
/**
 * Expense Categories Section Query
 */
public class DonerSectionQuery extends JFrame {
    ...
    public void Connect() {
        try {
            ...
            Logger.getLogger(DonerSectionQuery.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private void initComponents() {
        setTitle("Expense Categories Section Query");
        getContentPane().setLayout(null);

        jLabel3 = new JLabel("Expense-Categories-Section-Query");
        jLabel3.setFont(new Font("Comic Sans MS", Font.BOLD, 19));
        jLabel3.setForeground(new Color(0, 51, 51));
        jLabel3.setBounds(280, 40, 620, 50);
        ...
        jRadioButton3.addActionListener(e -> {
            if (jRadioButton3.isSelected())
                loadTable("SELECT * FROM expensecategory", null);
        });

        jRadioButton1.addActionListener(e -> {
            String st = JOptionPane.showInputDialog(this, "Enter Category Name to Fetch:");
            if (st != null && !st.isEmpty())
                loadTable("SELECT * FROM expensecategory WHERE categoryname=?", st);
        });

        jRadioButton2.addActionListener(e -> {
            String st = JOptionPane.showInputDialog(this, "Enter Category Id to Fetch:");
            if (st != null && !st.isEmpty())
                loadTable("SELECT * FROM expensecategory WHERE categoryid=?", st);
            else
